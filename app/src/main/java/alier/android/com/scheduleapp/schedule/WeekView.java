package alier.android.com.scheduleapp.schedule;
...
import alier.android.com.scheduleapp.R;

import static alier.android.com.scheduleapp.schedule.WeekViewUtil.isSameDay;
import static alier.android.com.scheduleapp.schedule.WeekViewUtil.today;
...
public class WeekView extends View {
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            switch (mCurrentFlingDirection) {
                case LEFT:
                case RIGHT:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) (velocityX * mXScrollingSpeed), 0, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * mHourLimit + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
                case VERTICAL:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, 0, (int) velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * mHourLimit + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
            }
    ...
    // TODO limit LEFT / RIGHT Scroll from Monday to SUNDAY, Now it is scolling infinitive
    private boolean mIsDragging;
    private float mLastTouchX, mLastTouchY, dx = 0, dy = 0, x = 0, y = 0;
    ...
    private void initTextTimeWidth() {
        mTimeTextWidth = 0;
        for (int i = 0; i < mHourLimit; i++) {
            // Measure time string and get max width.
            String time = getDateTimeInterpreter().interpretTime(i + 8, 0);
            if (time == null)
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        for (int i = 0; i < mHourLimit; i++) {
            ...
            String time_15 = getDateTimeInterpreter().interpretTime(i + 8, 15);
            String time_30 = getDateTimeInterpreter().interpretTime(i + 8, 30);
            String time_45 = getDateTimeInterpreter().interpretTime(i + 8, 45);
            if (time_00 == null)
                throw new IllegalStateException("A DateTimeInterpreter must not return null time");
            if (top < getHeight()) {
                canvas.drawText(time_00, mTimeTextWidth + mHeaderColumnPadding, top + mTimeTextHeight + mHourHeight * 0 / 4, mTimeTextPaint);
                canvas.drawText(time_15, mTimeTextWidth + mHeaderColumnPadding, top + mTimeTextHeight + mHourHeight * 1 / 4, mTimeTextPaint);
                canvas.drawText(time_30, mTimeTextWidth + mHeaderColumnPadding, top + mTimeTextHeight + mHourHeight * 2 / 4, mTimeTextPaint);
                canvas.drawText(time_45, mTimeTextWidth + mHeaderColumnPadding, top + mTimeTextHeight + mHourHeight * 3 / 4, mTimeTextPaint);
            }
        }
    }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight = Math.max(mMinHourHeight,
                    (int) ((getHeight() - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom) / mHourLimit));
        ...
        if (mCurrentOrigin.y < getHeight() - mHourHeight * mHourLimit - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight / 2)
            mCurrentOrigin.y = getHeight() - mHourHeight * mHourLimit - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight / 2;
        ...
        float[] hourLines = new float[lineCount * 12];
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            // Prepare the separator lines for every 15 minutes.
            int i = 0;
            for (int hourNumber = 0; hourNumber < mHourLimit; hourNumber++) {
            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && !mEventRects.get(i).event.isAllDay()) {
                    ...
                    float top = mHourHeight * 24 * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderHeight +
                            mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 + mEventMarginVertical;

                    // Calculate bottom.
                    float bottom = mEventRects.get(i).bottom;
                    bottom = mHourHeight * 24 * bottom / 1440 + mCurrentOrigin.y + mHeaderHeight + mHeaderRowPadding * 2 +
                ...
                public String interpretTime(int hour, int minute) {
                    ...
                    try {
                        ...
                        if (minute == 0) {
                            ...
                            sdf = !DateFormat.is24HourFormat(getContext()) ? new SimpleDateFormat("  :mm", Locale.getDefault()) : new SimpleDateFormat("hh a", Locale.getDefault());
                        }
     ...
     * </p>
     *
     * @param length Supported values are {@link alier.android.com.scheduleapp.schedule.WeekView#LENGTH_SHORT} and
     *               {@link alier.android.com.scheduleapp.schedule.WeekView#LENGTH_LONG}.
     */
    @Deprecated
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        String firstDay = new SimpleDateFormat("EEE").format(mFirstVisibleDay.getTime());
        String lastDay = new SimpleDateFormat("EEE").format(mLastVisibleDay.getTime());
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (event.getAction() == MotionEvent.ACTION_UP && !mIsZooming && mCurrentFlingDirection == Direction.NONE) {
                    if (mCurrentScrollDirection == Direction.RIGHT || mCurrentScrollDirection == Direction.LEFT) {
                        if ((event.getX() - mLastTouchX > 0 && firstDay.equals("Mon")) || (event.getX() - mLastTouchX < 0 && lastDay.equals("Sun"))) {
                            return false;
                        }
                        goToNearestOrigin();
                    }
    ...
    public interface EventLongPressListener {
        /**
         * Similar to {@link alier.android.com.scheduleapp.schedule.WeekView.EventClickListener} but with a long press.
         *
         * @param event:     event clicked.
         * @param eventRect: view containing the clicked event.
         */
    ...
    public interface EmptyViewLongPressListener {
        /**
         * Similar to {@link alier.android.com.scheduleapp.schedule.WeekView.EmptyViewClickListener} but with long press.
         *
